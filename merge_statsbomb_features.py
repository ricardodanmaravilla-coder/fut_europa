...
Prioriza coincidencia exacta; fuzzy sólo misma fecha/orden y score >=0.88. Conserva
xThreat transferido desde Wyscout en columnas explícitas para experimentación OOS.
"""
import difflib, glob, os, unicodedata
import numpy as np
import pandas as pd

SB='data/statsbomb_xg_matches.csv'
ADV_COLS=['Pases_SB_Local','Pases_SB_Visita','Presiones_SB_Local','Presiones_SB_Visita','Posesiones_SB_Local','Posesiones_SB_Visita','PPDA_Local','PPDA_Visita']
...
def main():
    ...
    total=exact=fuzzy=transfer=0
    for path in glob.glob('data/historico_*.csv'):
        if 'statsbomb' in path: continue
        df=pd.read_csv(path); df['Fecha']=df['Fecha'].astype(str)
        cols=ADV_COLS+['xThreat_Wyscout_Local','xThreat_Wyscout_Visita','StatsBomb_match_score','StatsBomb_match_id','xThreat_Source']
        for c in cols:
            if c not in df.columns: df[c]=np.nan
        bydate={d:g for d,g in sb.groupby('Fecha')}; matched=0
        for i,r in df.iterrows():
            ...
            df.at[i,'StatsBomb_match_score']=round(bestscore,4); df.at[i,'StatsBomb_match_id']=best.get('StatsBomb_match_id')
            for c in ADV_COLS:
                if c in best.index: df.at[i,c]=best.get(c)
            src=str(best.get('xThreat_Source',''))
            df.at[i,'xThreat_Source']=src
            if 'Wyscout public' in src and pd.notna(best.get('xThreat_Local')) and pd.notna(best.get('xThreat_Visita')):
                df.at[i,'xThreat_Wyscout_Local']=best.get('xThreat_Local'); df.at[i,'xThreat_Wyscout_Visita']=best.get('xThreat_Visita'); transfer+=1
    ...
    print('MERGE_SB_OK',{'matched':total,'exact':exact,'fuzzy':fuzzy,'wyscout_xt_transfer':transfer})
