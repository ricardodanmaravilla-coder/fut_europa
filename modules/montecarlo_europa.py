...
MC_PROFILE_PARQUET = Path("data/mc_profiles_europa.parquet")
_MC_LOCK = RLock()
_MC_PROFILE_MEM = None
_MC_RUNTIME_MEM = {}
...
def _team_recent(df, team, venue=None, n=12):
    ...
    if venue == "home":
        vx = x[x["Local"] == team].tail(max(6, n // 2))
    elif venue == "away":
        vx = x[x["Visitante"] == team].tail(max(6, n // 2))
    else:
        vx = x.iloc[0:0]
    venue_p = _rows_profile(vx, team)
    if overall is None:
        return venue_p
    if venue_p is None or venue_p["n"] < 3:
        return overall
    out = {k: 0.65 * venue_p[k] + 0.35 * overall[k] for k in ("xgf", "xga", "cf", "ca", "cards")}
    out["n"] = overall["n"]
    out["venue_n"] = venue_p["n"]
    return out


def _data_version(df):
    try:
...
def build_mc_profile_rows(df, league):
    """Build the exact team/venue inputs currently consumed by Monte Carlo."""
    if df is None or df.empty:
        return []
    teams = sorted(set(df["Local"].dropna().astype(str)).union(set(df["Visitante"].dropna().astype(str))))
    version = _data_version(df)
    rows = []
    for team in teams:
        for venue in ("home", "away"):
            p = _team_recent(df, team, venue)
            if not p:
                continue
            rows.append({
                "league": str(league), "data_version": version, "team": team, "venue": venue,
                "xgf": float(p["xgf"]), "xga": float(p["xga"]), "cf": float(p["cf"]),
                "ca": float(p["ca"]), "cards": float(p["cards"]), "n": int(p.get("n", 0)),
                "venue_n": int(p.get("venue_n", 0)),
            })
    return rows


def _load_profile_store():
    global _MC_PROFILE_MEM
    with _MC_LOCK:
        if _MC_PROFILE_MEM is not None:
            return _MC_PROFILE_MEM
        try:
            _MC_PROFILE_MEM = pd.read_parquet(MC_PROFILE_PARQUET) if MC_PROFILE_PARQUET.exists() else pd.DataFrame()
        except Exception:
            _MC_PROFILE_MEM = pd.DataFrame()
        return _MC_PROFILE_MEM


def _cached_profile(df, team, venue):
    version = _data_version(df)
    key = (version, str(team), str(venue))
    with _MC_LOCK:
        if key in _MC_RUNTIME_MEM:
            return _MC_RUNTIME_MEM[key]
    store = _load_profile_store()
    if not store.empty:
        x = store[(store["data_version"].astype(str) == version) &
                  (store["team"].astype(str) == str(team)) &
                  (store["venue"].astype(str) == str(venue))]
        if len(x) == 1:
            r = x.iloc[0]
            p = {k: float(r[k]) for k in ("xgf", "xga", "cf", "ca", "cards")}
            p["n"] = int(r.get("n", 0))
            p["venue_n"] = int(r.get("venue_n", 0))
            with _MC_LOCK:
                _MC_RUNTIME_MEM[key] = p
            return p
    # Safety fallback preserves predictions if the image cache is stale/missing.
    p = _team_recent(df, team, venue)
    with _MC_LOCK:
        _MC_RUNTIME_MEM[key] = p
    return p
...
def simular_partido_europa(local, visita, df_historico, elo_local, elo_visita,
    ...
    pl = _cached_profile(df_historico, local, "home") or {"xgf":1.35,"xga":1.25,"cf":5.0,"ca":4.8,"cards":2.0,"n":0}
    pv = _cached_profile(df_historico, visita, "away") or {"xgf":1.15,"xga":1.35,"cf":4.5,"ca":5.0,"cards":2.1,"n":0}
    ...
    lambda_l=float(np.clip(base_l*np.exp(0.10*elo_diff),0.25,3.50))
    lambda_v=float(np.clip(base_v*np.exp(-0.10*elo_diff),0.20,3.20))
    corners_l=float(np.clip((0.60*pl["cf"]+0.40*pv["ca"])*np.exp(0.025*elo_diff),1.5,9.0))
    corners_v=float(np.clip((0.60*pv["cf"]+0.40*pl["ca"])*np.exp(-0.025*elo_diff),1.5,9.0))
