...
class PredictorMLEuropa:
    ...
    def _best_temperature(model, X_cal, y_cal):
        ...
        raw = model.predict_proba(X_cal); classes = list(model.classes_)
    ...
    def _fit_with_temporal_calibration(self, model, X, y):
        ...
        if cut<n:
            model.fit(X[:cut],y[:cut]); t=self._best_temperature(model,X[cut:],y[cut:])
    ...
    def entrenar(self, df_historico):
        ...
        for _,row in df.iterrows():
            ...
            loc,vis=str(loc).strip(),str(vis).strip(); date=row.get("_fecha")
            sl=states.setdefault(loc,self._new_state()); sv=states.setdefault(vis,self._new_state())
            ...
            if pl["games"]>=4 and pv["games"]>=4:
                X.append(self._features(pl,pv,elo_l,elo_v)); y1.append(2 if gl>gv else (1 if gl==gv else 0))
                yg.append(int(gl+gv>2.5)); yc.append(int(self._safe(row.get("Corners_Local"),0)+self._safe(row.get("Corners_Visita"),0)>9.5))
            ...
            pts_l=3.0 if gl>gv else (1.0 if gl==gv else 0.0); pts_v=3.0 if gv>gl else (1.0 if gl==gv else 0.0)
            for st,xgf,xga,gf,ga,sf,sa,cf,ca,cards,pts in [
                (sl,xgl,xgv,gl,gv,sotl,sotv,cl,cv,cal,pts_l),(sv,xgv,xgl,gv,gl,sotv,sotl,cv,cl,cav,pts_v)]:
                vals={"xg_for":xgf,"xg_against":xga,"goals_for":gf,"goals_against":ga,
                      "sot_for":sf,"sot_against":sa,"corners_for":cf,"corners_against":ca,"cards":cards,"points":pts}
                st["games"]+=1
                for key,val in vals.items():
                    val=float(val); st[key]=self._ema(st.get(key),val); st[f"sum_{key}"]+=val
                if pd.notna(date): st["last_date"]=date
            exp_l=self._elo_expected(elo_l,elo_v); score_l=1.0 if gl>gv else (0.5 if gl==gv else 0.0)
            k=22.0*(1.0+0.12*min(abs(gl-gv),4.0)); delta=k*(score_l-exp_l)
            ratings[loc],ratings[vis]=elo_l+delta,elo_v-delta
        if len(X)<250 or len(set(y1))<3: return False
        X=np.asarray(X,dtype=float); y1,yg,yc,yt=map(np.asarray,(y1,yg,yc,yt))
        ...
        if len(X)-cut>=80:
            ...
            tmp.fit(X[:cut],y1[:cut]); pm=_temperature(tmp.predict_proba(X[cut:]),self.temp_1x2)
            idx=[]; marr=[]; target=[]
            for j,mk in enumerate(markets[cut:]):
                if mk is not None: idx.append(j); marr.append(mk); target.append(y1[cut+j])
            if len(idx)>=60:
                pmv=pm[np.asarray(idx)]; marr=np.asarray(marr); target=np.asarray(target)
        ...
        self.n_train=len(X); self.entrenado=True; return True
    ...
    def predecir_mercados_completos(self, local, visita, goles_sim_l=None, goles_sim_v=None,
                                    elo_local=1500, elo_visita=1500, cuotas_1x2=None, fecha_partido=None):
        ...
        p1=self._ordered_probs(self.modelo_1x2,raw1,[0,1,2]); used_market=False
        if cuotas_1x2:
            mk=_no_vig_1x2(cuotas_1x2.get("1"),cuotas_1x2.get("X"),cuotas_1x2.get("2"))
            if mk is not None and self.market_model_weight<1.0:
                p1=_normalize_rows([self.market_model_weight*p1+(1-self.market_model_weight)*mk])[0]; used_market=True
        ...
        return {
            ...
            "Meta":{"train_rows":self.n_train,"pregame_only":True,"temperature_1x2":self.temp_1x2,
                    "market_model_weight":self.market_model_weight,"market_blend_used":used_market,
                    "calibration_rows":self.calibration_rows,"rest_local":pl["rest_days"],"rest_visita":pv["rest_days"]},
        }
