...
class PredictorMLEuropa:
    def __init__(self):
        ...
        self.modelo_goles = RandomForestClassifier(
            n_estimators=320, max_depth=10, min_samples_leaf=8,
            max_features=0.85, random_state=43, class_weight="balanced_subsample"
        )
        self.modelo_corners = RandomForestClassifier(
            n_estimators=300, max_depth=10, min_samples_leaf=9,
            max_features=0.85, random_state=44, class_weight="balanced_subsample"
        )
        self.modelo_tarjetas = RandomForestClassifier(
            n_estimators=300, max_depth=10, min_samples_leaf=9,
            max_features=0.85, random_state=45, class_weight="balanced_subsample"
        )
        self.stats_equipos = {}
        self.raw_states = {}
    ...
    def entrenar(self, df_historico):
        ...
        for _, row in df.iterrows():
            ...
            if pl["games"] >= 4 and pv["games"] >= 4:
                ...
                for line in LINE_GRIDS["goles"]:
                    if abs(goals_total - line) < 1e-9:
                        continue
                    Xg.append(self._line_features(base, line))
                    yg.append(int(goals_total > line))
                for line in LINE_GRIDS["corners"]:
                    if abs(corners_total - line) < 1e-9:
                        continue
                    Xc.append(self._line_features(base, line))
                    yc.append(int(corners_total > line))
                for line in LINE_GRIDS["tarjetas"]:
                    if abs(cards_total - line) < 1e-9:
                        continue
                    Xt.append(self._line_features(base, line))
                    yt.append(int(cards_total > line))
            ...
            for st, xgf, xga, gf, ga, sf, sa, cf, ca, cards, pts in [
                ...
                vals = {
                    "xg_for": xgf, "xg_against": xga, "goals_for": gf, "goals_against": ga,
        ...
        if len(X1) < 250 or len(set(y1)) < 3:
            return False
        if min(len(Xg), len(Xc), len(Xt)) < 500:
            return False
        ...
        cut = int(len(X1) * 0.80)
        if len(X1) - cut >= 80:
            tmp = ExtraTreesClassifier(
                n_estimators=280, max_depth=10, min_samples_leaf=7,
            ...
            for j, mk in enumerate(markets[cut:]):
                if mk is not None:
                    idx.append(j)
                    marr.append(mk)
                    target.append(y1[cut + j])
            if len(idx) >= 60:
                pmv = pm[np.asarray(idx)]
    ...
    def predecir_mercados_completos(self, local, visita, goles_sim_l=None, goles_sim_v=None,
        ...
        if isinstance(lineas_casino, dict):
            for k in lineas:
                try:
                    if lineas_casino.get(k) is not None:
                        lineas[k] = float(lineas_casino[k])
                except Exception:
                    pass
        ...
        return {
            "Resultado_1X2": {
                "Gana Local": round(p1[2] * 100, 1),
                "Empate": round(p1[1] * 100, 1),
                "Gana Visita": round(p1[0] * 100, 1),
            },
            "Goles_Over_Under": {
                f"Over {gl:g}": round(pg[1] * 100, 1),
                f"Under {gl:g}": round(pg[0] * 100, 1),
            },
            "Corners_Totales": {
                f"Over {cl:g} Corners": round(pc[1] * 100, 1),
                f"Under {cl:g} Corners": round(pc[0] * 100, 1),
            },
            "Tarjetas_Totales": {
                f"Over {tl:g} Tarjetas": round(pt[1] * 100, 1),
                f"Under {tl:g} Tarjetas": round(pt[0] * 100, 1),
            },
            "Meta": {
                "train_rows": self.n_train,
                "train_rows_line_aware": self.n_train_lines,
                "pregame_only": True,
                "line_aware": True,
                "lineas_modeladas": lineas,
                "temperature_1x2": self.temp_1x2,
                "temperature_goles": self.temp_goles,
                "temperature_corners": self.temp_corners,
                "temperature_tarjetas": self.temp_tarjetas,
                "market_model_weight": self.market_model_weight,
                "market_blend_used": used_market,
                "calibration_rows": self.calibration_rows,
                "rest_local": pl["rest_days"],
                "rest_visita": pv["rest_days"],
            },
        }
